          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
            {codeBlogs.map((blog, index) => (
              ...
              >
                <Link
                  to={`/blog/${blog._id}`}
                  className="group relative bg-zinc-900/80 backdrop-blur-xl border border-zinc-700/50 rounded-4xl p-8 shadow-2xl hover:shadow-3xl hover:shadow-cyan-500/20 hover:-translate-y-3 transition-all duration-500 overflow-hidden h-full"
                >
                    ...
                    >
                      {blog.adminPhoto ? (
                        ...
                        <div className="w-14 h-14 bg-gradient-to-r from-zinc-700 to-zinc-600 rounded-2xl flex items-center justify-center text-xl font-bold text-zinc-400 flex-shrink-0">
                          {blog.adminName?.charAt(0)?.toUpperCase()}
                        </div>
                      )}
