...
export default function Creators() {
  ...
  const getRoleIcon = (role) => {
    switch (role?.toLowerCase()) {
      case 'admin': return <CodeBracketIcon className="w-8 h-8 text-emerald-400" />;
      case 'user': return <UserIcon className="w-8 h-8 text-cyan-400" />;
    ...
    <div className="min-h-screen py-24 px-6 md:px-12 lg:px-24 bg-gradient-to-b from-zinc-950 via-black/90 to-zinc-900 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
            ...
            {admin.map((user, index) => (
              ...
              >
                ...
                >
                  ...
                  />
                  {/* Status Ring */}
                  <div className="absolute -inset-1 bg-gradient-to-r from-emerald-400 to-cyan-500 rounded-3xl blur opacity-75 animate-spin-slow" />
                </motion.div>
                ...
                <motion.h3 
                  initial={{ opacity: 0, y: 20 }}
                  whileHover={{ y: -8 }}
                  className="text-2xl font-black text-zinc-100 text-center mb-3 bg-gradient-to-r from-zinc-100 to-zinc-200 bg-clip-text group-hover:from-emerald-400 group-hover:to-cyan-400 transition-all duration-500 leading-tight"
                >
                  {user.name}
                </motion.h3>
                ...
                <motion.div
                  ...
                  className="flex items-center justify-center gap-2 mb-4 p-3 bg-zinc-800/50 backdrop-blur-sm rounded-2xl border border-zinc-600/50 mx-4"
                >
        ...
        <motion.div
          ...
          className="text-center mt-24 pt-20 border-t-4 border-zinc-800/50"
        >
          <motion.button
            whileHover={{ scale: 1.05, boxShadow: "0 25px 50px rgba(6, 182, 212, 0.4)" }}
            whileTap={{ scale: 0.95 }}
            className="px-12 py-6 bg-gradient-to-r from-emerald-500 via-cyan-500 to-blue-600 text-white font-black text-xl rounded-4xl shadow-2xl hover:shadow-emerald-500/50 border border-transparent hover:from-emerald-600 hover:via-cyan-600 hover:to-blue-700 transition-all duration-400"
          >
            👋 Become a Creator
          </motion.button>
        </motion.div>
